...
struct conicALMTOPP2
{
  ...
  int get_conic3_pri(const Ev3 &v, Ev3 &prj)
  {
    double v0 = v(0);
  ...
  static double obj_fcn_alm(void *ptr_obj,const Evx &x,Evx &grad)
  {
    ...
    for(int k=0; k<n_cones;++k){
      //1/p(u)
      p.get_conic3_pri((1.0/rho)*mius[k]-As[k]*x-bs[k],prj);
      value_cone +=0.5*rho*prj.squaredNorm();
      grad-=rho*As[k].transpose() * prj;//A->锥
    }

    Evx res_ieq=((1/rho)*eta+Aieq*x-bieq).cwiseMax(zeros_ieq);
  ...
  double get_alm_error(const Evx &x,double &err_cons,double &err_prec)
  {
    ...
    for(int k=0;k<n_cones;++k)
    {
      get_conic3_pri((1.0/pa2.alm_rho)*mius[k]-As[k]*x-bs[k],prj);
      double err_cone1=
                ...
                lbfgs::lbfgs_parameter_t pl)
  {
    ...
    {
      //inner layer optimization by lbfgs TODO 如何得到
      pl2.g_epsilon=
      ...
      for(int k=0;k<n_cones;++k)
      {
        get_conic3_pri(mius[k]-pa2.alm_rho*(As[k]*x+bs[k]),mius[k]);
      }
  ...
  // solve interface
  int solve(double &value,Evx &xa,Evx &xb,Evx &xc,Evx &xc)
  {
