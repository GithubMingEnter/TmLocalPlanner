...
struct lbfgs_parameter_t {
   ...
   *  where ||.||_inf is the infinity norm. The default value is 0.0.
...
inline int line_search_lewisoverton(
  ...
  // note the output x, f and g which satisfy the weak wolfe condition when the
  // function returns
  ...
  Eigen::VectorXd gp0(gp.size());
  double fp = cd.proc_evaluate(cd.instance, xp, gp0);
  int count = 0;
  while (count < param.max_linesearch) {
    x = xp + stp * s;
    f = cd.proc_evaluate(cd.instance, x, g);
    double s_alpha = fp - f + param.f_dec_coeff * stp * s.dot(gp);
    if (s_alpha < 0) {
      u = stp;
    } else {
      double c_alpha = s.dot(g) - param.s_curv_coeff * s.dot(gp);
      if (c_alpha < 0) {
        l = stp;
      } else {
        alpha_found = 0;
  ...
  // std::cout << "<line search steps:" << count << " size:" << stp
  //           << " value:" << f << std::endl;
      ...
      */
      if (0 < param.past) {
        ...
        if (param.past <= k) {
          ...
          rate =
              std::fabs(pf(k % param.past) - fx) / std::max(1.0, std::fabs(fx));
...
inline const char *lbfgs_strerror(const int err) {
  switch (err) {
    ...
    case LBFGS_STOP:
      return "Success: met stopping criteria (past f decrease less than "
             "delta).";
    ...
    case LBFGSERR_MAXIMUMLINESEARCH:
      return "Line search reaches the maximum try number, assumptions not "
             "satisfied or precision not achievable.";

    case LBFGSERR_MAXIMUMITERATION:
      return "The algorithm routine reaches the maximum number of iterations.";

    case LBFGSERR_WIDTHTOOSMALL:
      return "Relative search interval width is at least "
             "lbfgs_parameter_t::machine_prec.";
