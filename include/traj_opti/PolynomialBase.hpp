...
public:
    PolynomialBase(ros::NodeHandle nh) :
    nh_(nh)
    {
        ...
        if(is_debug_) ROS_INFO_STREAM("-----------Qi param----------------\n" << Qi_param_);

    }
    
    PolynomialBase(int order, int mini_order, bool debug):
    n(order),
    k(mini_order),
    is_debug_(debug){
...
void PolynomialBase::updateOptiParam(std::vector<Rectangle> corridors, Eigen::MatrixXd pos, Eigen::MatrixXd vel, Eigen::MatrixXd acc){
    ...
    //*********************************************************
    if(is_debug_) ROS_INFO_STREAM("[polyBase]: -------- create Q---------");
    m = corridors.size();
    ...
    for(int i=0; i<m-1; i++){
        ...
        x_lo = corridors[i].corss_rect->vertex(3, 0);
        x_up = corridors[i].corss_rect->vertex(2, 0);


        y_lo = corridors[i].corss_rect->vertex(3, 1);
        y_up = corridors[i].corss_rect->vertex(0, 1);
