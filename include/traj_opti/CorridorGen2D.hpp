...
{
    Eigen::MatrixXd vertex;//相当于4乘3的数组   vertex:4x3
    Eigen::Vector3d center; // the center of the Rectangle
    bool isValid; // is the corridor should be delte
    double t; // time allocated to this Rectangle
    Rectangle *corss_rect;
    ...
    */
    void setVertex( Eigen::MatrixXd vertex_, double resolution_){
        vertex = vertex_;
        vertex(1, 0) += resolution_ / 2;
        vertex(2, 0) += resolution_ / 2;

        vertex(0, 0) -= resolution_ / 2;
        vertex(3, 0) -= resolution_ / 2;

        vertex(0, 1) += resolution_ / 2;
        vertex(1, 1) += resolution_ / 2;

        vertex(3, 1) -= resolution_ / 2;
        vertex(2, 1) -= resolution_ / 2;
    ...
    Rectangle(Eigen::MatrixXd vertex_){
        vertex = vertex_;
        isValid = true;
        t = 0.0;
        corss_rect = nullptr;
    ...
    Rectangle(){
        ...
        vertex = Eigen::MatrixXd::Zero(4, 3);
...
private:
    ...
    Eigen::Vector3d start_point_, end_point_;
...
public:
    ...
    std::vector<Rectangle> corridorGeneration(std::vector<Eigen::Vector3d> path);
    Rectangle generateRect(Eigen::Vector3d pt); // the input pt is in world cori
    ~CorridorGen2D(){};
};
...
start_vel_(start_vel){
    ...
    n_.param<int>("corridor/inflat_len_per_iter", inflat_len_per_iter_, 3); // inflat_len_per_iter_ * resolution
    ...
    ROS_INFO("\033[1;32m ----> this is a [2D] [corridor] !! .\033[0m");
...
std::pair<Rectangle, bool> CorridorGen2D::inflateRect(Rectangle rect, Rectangle last_rect){
    ...
    */
    ...
    while(iter <= max_iter_nu_){
        ...
        if(is_debug_) ROS_INFO_STREAM(" ======================================== -- " << iter);
        ...
        if(is_debug_) ROS_INFO_STREAM("[X+] p1.x --> x_up: " << vertex_idx(1, 0) << " --> " << x_up);
        //     p1.x  ---> x_up
        // --------------------------------------> x
        for(id_x = vertex_idx(1, 0); id_x <= x_up; id_x += res_){
            ...
            for(id_y = vertex_idx(2, 1); id_y <= vertex_idx(1, 1); id_y += res_){
                ...
                Eigen::Vector3d p(id_x, id_y, 0);
                // if(is_debug_) ROS_INFO_STREAM("check point: " << p.transpose());
        ...
        if (collide)
        {
            ...
            vertex_idx(1, 0) = vertex_idx(2, 0) = id_x - res_; // if no collide id_x += res_; should - res_;
        }

        if(is_debug_) ROS_INFO_STREAM("[X+] ----p1.x x+ after inflate: " << vertex_idx(1, 0));
        ...
        if(is_debug_) ROS_INFO_STREAM("[X-] p0.x --> x_lo: " << vertex_idx(0, 0) << " --> " << x_lo);
        // x_lo <---- p0.x
        for(id_x = vertex_idx(0, 0); id_x >= x_lo; id_x -= res_){
            ...
            for(id_y = vertex_idx(0, 1); id_y >= vertex_idx(3, 1); id_y -= res_){
                ...
                Eigen::Vector3d p(id_x, id_y, 0);
                // if(is_debug_) ROS_INFO_STREAM("[X-] check point: " << p.transpose());
        ...
        if(is_debug_) ROS_INFO_STREAM("[X-] ----p0.x x- after inflate: " << vertex_idx(0, 0));
        ...
        if(is_debug_) ROS_INFO_STREAM("[Y+] p0.y --> y_up: " << vertex_idx(0, 1) << " --> " << y_up);
        // p0.y --> y_up
        ...
        if(collide){
            if(is_debug_) ROS_INFO_STREAM("[Y+]   ------  collide" );
            vertex_idx(0, 1) = max(id_y - 2*res_, vertex_idx(0, 1));
            vertex_idx(1, 1) = max(id_y - 2*res_, vertex_idx(1, 1));
        }else{
            if(is_debug_) ROS_INFO_STREAM("[Y+]   ------  no collide" );
            vertex_idx(0, 1) = vertex_idx(1, 1) = id_y - res_;
        }

        if(is_debug_) ROS_INFO_STREAM("[Y+] ----p0.y after inflate: " << vertex_idx(0, 1) );
        ...
        if(is_debug_) ROS_INFO_STREAM("[Y-] p3.y --> y_lo: " << vertex_idx(3, 1) << " --> " << y_lo);
        // p3.y --> y_lo
        ...
        if(is_debug_) ROS_INFO_STREAM("[Y-] ----p3.y after inflate: " << vertex_idx(3, 1));
        ...
        if(iter == 1){
            ...
            if(isContain(rectMax, last_rect)){
                if(is_debug_) ROS_INFO_STREAM(" ---- contain check-----");
...
std::vector<Rectangle> CorridorGen2D::corridorGeneration(std::vector<Eigen::Vector3d> path){
    ...
    for(size_t i=0; i< path.size(); i++){
        ...
        rect_list.push_back(result.first);
    ...
    for(size_t i=1; i<rect_list_new.size(); i++){
        ...
        rect_list_new[i-1].corss_rect = cross_rect;
    }
    ...
    ROS_INFO_STREAM("\033[1;32m[corridor] | corridor gen time: " << (ros::Time::now() - corridor_start_time).toSec() * 1000 << " (ms) \033[0m");
...
void CorridorGen2D::calRectCross(Rectangle r1, Rectangle r2, Rectangle* r3){
    ...
    if(max(r1.vertex(3, 0), r2.vertex(3, 0)) < min(r1.vertex(1, 0), r2.vertex(1, 0)) &&
       max(r1.vertex(3, 1), r2.vertex(3, 1)) < min(r1.vertex(1, 1), r2.vertex(1, 1)) ){
        ...
        r3->vertex.row(0) = Eigen::Vector3d(p3_x, p1_y, 0);
        r3->vertex.row(1) = Eigen::Vector3d(p1_x, p1_y, 0);
        r3->vertex.row(2) = Eigen::Vector3d(p1_x, p3_y, 0);
        r3->vertex.row(3) = Eigen::Vector3d(p3_x, p3_y, 0);
        r3->setBox();
       }
}
...
std::vector<Rectangle> CorridorGen2D::simpleCorridor(std::vector<Rectangle> rect_list){
    ...
    new_rect_list.push_back(this_rect);
    ...
    if(!isPointInCorridor(end_point_, new_rect_list.back()))
        new_rect_list.push_back(rect_list.back());

    if(isPointInCorridor(start_point_, new_rect_list[1]))
        new_rect_list.erase(new_rect_list.begin());

    if(is_debug_) ROS_INFO_STREAM("[corridor] | the start point in second rect: " << isPointInCorridor(start_point_, new_rect_list[1]));
...
/*   
    ...
    p3--------p2   -----------> x
*/
Rectangle CorridorGen2D::generateRect(Eigen::Vector3d pt){
    Rectangle rect;
    rect.center(0) = pt(0);
    rect.center(1) = pt(1);
    rect.center(2) = pt(2);
    ...
    rect.vertex.row(0) = Eigen::Vector3d(x_lo, y_up, 0);
    rect.vertex.row(1) = Eigen::Vector3d(x_up, y_up, 0);
    rect.vertex.row(2) = Eigen::Vector3d(x_up, y_lo, 0);
    rect.vertex.row(3) = Eigen::Vector3d(x_lo, y_lo, 0);
...
/*   
        ...
        vertex(0, 0) is p0.x vector.row(0) p0
*/
bool CorridorGen2D::isPointInCorridor(Eigen::Vector3d p, Rectangle rect){
    
    Eigen::Vector3d p0p3 = rect.vertex.row(3) - rect.vertex.row(0);
    Eigen::Vector3d p0p  = p.transpose() - rect.vertex.row(0);
    Eigen::Vector3d p0p1 = rect.vertex.row(1) - rect.vertex.row(0);
    Eigen::Vector3d p2p1 = rect.vertex.row(1) - rect.vertex.row(2);
    Eigen::Vector3d p2p  = p.transpose() - rect.vertex.row(2);
    Eigen::Vector3d p2p3 = rect.vertex.row(3) - rect.vertex.row(2);

    // std::cout << "p0p3.dot(p0p) :" << p0p3.dot(p0p) << std::endl;
    
    return p0p3.dot(p0p) >= 0 && p0p1.dot(p0p) >= 0 && p2p1.dot(p2p) >= 0 && p2p3.dot(p2p) >=0;
}
...
void CorridorGen2D::timeAllocation(std::vector<Rectangle> & corridor){
    ...
    for(size_t i=0; i<corridor.size(); i++){
        if(corridor[i].corss_rect){ 
            points.push_back(corridor[i].corss_rect->center);
        }
    }
    ...
    for (int k = 0; k < (int)points.size() - 1; k++)
    {
        ...
        double D    = d.norm();                  
        double V0   = v0.dot(d / D);   // 将速度向连线方向投影,沿线的速度          
        double aV0  = fabs(V0);   // 速度绝对值               
