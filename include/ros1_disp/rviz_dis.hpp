...
struct colorSimp {
  ...
  double b;
  double a;
  colorSimp(double r0 = 0.5, double g0 = 0.5, double b0 = 0.5, double a0 = 1.0) : r(r0), g(g0), b(b0), a(a0) {}
  ~colorSimp() {}
};
...
struct rviz1DisSimp {
  ...
  int add_colored_path_strip2d(int id, Mat& path, Vec& val, double val_min, double val_max, double width,
                               double height, colorSimp c_min = colorSimp(1.0, 1.0, 0.2, 1.0), colorSimp c_max= colorSimp(0.8, 0.2, 0.2, 1.0) ) {
    visualization_msgs::Marker m = get_default_marker(id);
    m.type = visualization_msgs::Marker::LINE_STRIP;
    m.color.r = 1.0;
    m.color.g = 1.0;
