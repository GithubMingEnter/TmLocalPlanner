...
class following_path:
    def __init__(self):
        ...
        self.Pose = []
        self.path_pose = rospy.Subscriber('/topp_path/traj', Path, self.callback_read_path, queue_size=1)
        self.path_info = []
        self.Goal = []
    ...
    def pub_zero(self):
        ...
        twist_z.linear.x=0
        twist_z.angular.z=0
        self.navigation_input.publish(twist_z)
    ...
    def callback_read_current_position(self, data):
        ...
        if not len(self.path_info) == 0:
            ...
            twist_control=Twist()
            twist_control.linear.x=VELOCITY
            twist_control.angular.z=angle # VELOCITY* np.tan(angle)/wheelbase   
            self.navigation_input.publish(twist_control)
        else:
            self.pub_zero()
