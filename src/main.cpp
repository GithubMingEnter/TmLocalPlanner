...
void TmLocalPlanner::controlTimerCallback(const ros::TimerEvent &timer_event)
{
  ...
  if (b_vehicle_state && b_traj)
  {
    ...
    if (PhrAlm_)
    {
      ...
      current_time = times_ * dt_;
      // std::ofstream FoutC("/home/ming/ros1_workspace/mpc-car-tutorial-master/time_phrmpc.txt",std::ios::app);
      // writefile(FoutC,current_time,solve_time);
    }
    else
    {
      if (nmpc_)
      {
        ...
        current_time = times_ * dt_;
        // std::ofstream FoutC("/home/ming/ros1_workspace/mpc-car-tutorial-master/time_nmpc.txt",std::ios::app);
        // writefile(FoutC,current_time,solve_time);
      }
      else
      {
        ...
        current_time = times_ * dt_;
        // std::ofstream FoutC("/home/ming/ros1_workspace/mpc-car-tutorial-master/time_lmpc.txt",std::ios::app);
        // writefile(FoutC,current_time,solve_time);
    ...
    // cmd_vel
    cmd_vel_msg.linear.x = state_(3) + (u(0) * (dt_ + solve_time));
    cmd_vel_msg.linear.y = 0;
    cmd_vel_msg.linear.z = 0;

    cmd_vel_msg.angular.x = 0;
    cmd_vel_msg.angular.y = 0;
    cmd_vel_msg.angular.z = tan(u(1)) * state_(3) / wheelbase_length_;
